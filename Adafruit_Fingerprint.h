...
#define FINGERPRINT_DOWNFAIL 0xe
...
#define DEFAULTTIMEOUT 10000  // milliseconds
...
class Adafruit_Fingerprint {
 public:
  ...
  uint8_t image2Tz(uint8_t slot = 0x01);
  uint8_t createModel(void);

  uint8_t emptyDatabase(void);
  uint8_t setSysParaBaud(void);
  uint8_t setSysParaSize(void);
  ...
  uint8_t uploadTemplateOne(void);
  uint8_t uploadTemplateTwo(void);
  uint8_t uploadTemplateThree(void);
  uint8_t uploadTemplateFour(void);
  uint8_t uploadTemplateFive(void);
  uint8_t deleteModel(uint16_t id);
  uint8_t fingerFastSearch(void);
  uint8_t getMatch(void);
  uint8_t getTemplateCount(void);
  void writePacket(uint32_t addr, uint8_t packettype, uint16_t len, uint8_t *packet);
  void writeDataPacket(uint32_t addr, uint8_t packettype, uint16_t len, uint8_t *packet);
